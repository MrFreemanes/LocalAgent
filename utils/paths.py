...
def path_to_dir(name: str = 'LocalAgent') -> str:
...
def path_to_config(name_file: str = 'config.json') -> str:
    """Путь к файлам в папке config."""
    log_dir = os.path.join(path_to_dir(), 'config')
    os.makedirs(log_dir, exist_ok=True)
    return os.path.join(log_dir, name_file)
